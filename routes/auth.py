...
def login():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # ورود دائمی روی دستگاه‌های مشترک کارگاه خطرناک است.
...
def change_password():
    ...
    if request.method == 'POST':
        ...
        password_error = validate_password_strength(new_password, current_user.username)
        if password_error:
        ...
        if change_user_password(current_user.id, new_password, clear_must_change=True):
            ...
            if updated_user:
                ...
                current_user.session_version = updated_user.session_version
                session['auth_version'] = updated_user.session_version
