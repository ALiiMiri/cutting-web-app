...
def _safe_backup_path(backup_filename):
    filename = os.path.basename(str(backup_filename or ""))
    if filename != backup_filename or not filename.startswith("backup_") or not filename.endswith(".db"):
        raise ValueError("نام فایل بکاپ معتبر نیست.")
...
def create_backup(reason="manual", user="admin", metadata=None):
    ...
    try:
        ...
        backup_filename = f"backup_{timestamp}_{uuid.uuid4().hex[:8]}.db"
        ...
        backup_conn = sqlite3.connect(partial_path, timeout=30)
        try:
            with backup_conn:
        ...
        # Keep the metadata filename paired with the unique database filename.
        # The previous timestamp-only name meant list_backups() could not find
        # metadata for backups whose database name also contains a UUID suffix.
        ...
        backup_info = {
            ...
            "schema_migrations": verify_result.get("schema_migrations", []),
            "sha256": verify_result.get("sha256"),
        ...
        if Config.BACKUP_MIRROR_DIR:
            os.makedirs(Config.BACKUP_MIRROR_DIR, exist_ok=True)
            mirror_path = os.path.join(Config.BACKUP_MIRROR_DIR, backup_filename)
            shutil.copy2(backup_path, mirror_path)
            if _sha256(mirror_path) != verify_result["sha256"]:
                raise RuntimeError("کپی خارج از سرور با فایل اصلی یکسان نیست.")
            shutil.copy2(metadata_path, os.path.join(Config.BACKUP_MIRROR_DIR, os.path.basename(metadata_path)))
        
        print(f"✓ بکاپ با موفقیت ایجاد شد: {backup_path}")
        return True, backup_path
        
    except Exception as e:
        for candidate in (locals().get("partial_path"), locals().get("backup_path")):
            if candidate and candidate.endswith(".partial") and os.path.exists(candidate):
                os.remove(candidate)
...
def verify_backup(backup_path):
    ...
    """
    try:
        conn = sqlite3.connect(f"file:{os.path.abspath(backup_path)}?mode=ro", uri=True)
        cursor = conn.cursor()
        ...
        if foreign_key_errors:
            conn.close()
            return {"valid": False, "error": f"Foreign key check failed: {len(foreign_key_errors)} errors"}
        
        # شمارش رکوردها در جداول اصلی
        table_counts = {}
        important_tables = [
            "projects", "doors", "profile_types", "inventory_items",
            "inventory_pieces", "inventory_operations", "inventory_waste_items",
            "custom_columns", "users",
        ]
        ...
        migrations = []
        try:
            migrations = [row[0] for row in cursor.execute("SELECT name FROM schema_migrations ORDER BY name")]
        except sqlite3.OperationalError:
            pass
        conn.close()
        return {
...
def list_backups():
    ...
    for filename in os.listdir(BACKUP_DIR):
        if filename.endswith(".db") and filename.startswith("backup_"):
            ...
            # Backups created briefly by the old naming scheme stored metadata
            # without the UUID suffix. Read those files for backward
            # compatibility, while all new backups use the exact paired name.
            stem_parts = os.path.splitext(filename)[0].rsplit("_", 1)
            legacy_metadata_path = (
                os.path.join(BACKUP_DIR, stem_parts[0] + ".json")
                if len(stem_parts) == 2 and len(stem_parts[1]) == 8
                else None
            )
            ...
            readable_metadata_path = metadata_path
            if not os.path.exists(readable_metadata_path) and legacy_metadata_path:
                readable_metadata_path = legacy_metadata_path

            if os.path.exists(readable_metadata_path):
                try:
                    with open(readable_metadata_path, 'r', encoding='utf-8') as f:
                        metadata = json.load(f)
                        backup_info.update(metadata)
                except (OSError, ValueError, TypeError):
                    pass

            # Templates and API consumers can rely on these fields even for
            # old backups or manually copied database files without metadata.
            backup_info["datetime"] = backup_info.get("datetime") or backup_info["created"]
            backup_info["reason"] = backup_info.get("reason") or "نامشخص"
...
def restore_backup(backup_filename, create_pre_restore_backup=True):
    ...
    """
    try:
        backup_path = _safe_backup_path(backup_filename)
    ...
    try:
        ...
        if not restored_check["valid"]:
            return False, f"بازیابی انجام شد اما کنترل سلامت شکست خورد: {restored_check['error']}"
        return True, "دیتابیس با موفقیت و بدون تغییر رمز کاربران بازگردانی شد."
...
def cleanup_old_backups(retention_days=None, monthly_retention_days=365):
    """
    ...
    for filename in os.listdir(BACKUP_DIR):
        if filename.endswith(".db") and filename.startswith("backup_"):
            ...
            candidates.append((filename, backup_path, file_time))

    monthly_representatives = {}
    for filename, backup_path, file_time in sorted(candidates, key=lambda row: row[2], reverse=True):
        monthly_representatives.setdefault((file_time.year, file_time.month), filename)

    monthly_cutoff = datetime.now() - timedelta(days=monthly_retention_days)
    for filename, backup_path, file_time in candidates:
        keep_as_monthly = (
            monthly_representatives.get((file_time.year, file_time.month)) == filename
            and file_time >= monthly_cutoff
        )
        if file_time < cutoff_date and not keep_as_monthly:
                try:
